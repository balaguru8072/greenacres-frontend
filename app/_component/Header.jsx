...
import { usePathname } from "next/navigation"; // ✅ usePathname add pannu
...
function Header() {
  ...
  const pathname = usePathname(); // ✅ Idha add pannu
  ...
  useEffect(() => {
    const getProfile = async () => {
      try {
        console.log("1. Calling profile...");
        const res = await axios.get(`${BASE_URL}/api/users/profile`, {
          withCredentials: true,
        });
        console.log("2. Profile success:", res.data);
        if (res.data.success) setUser(res.data.user);
      } catch (error) {
        console.log("3. Profile error:", error.response?.status, error.response?.data);
        setUser(null);
      }
    };
    getProfile();
